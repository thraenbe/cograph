...
function fail(message) {
  process.stderr.write(`CoGraph analyzer: ${message}\n`);
  process.exitCode = 3;
  return false;
}

/** Returns true when the graph was written. Never throws. */
function writeGraph(graph, out) {
  const stream = out || process.stdout;
  const nodes = graph.nodes ? graph.nodes.length : 0;
  const edges = graph.edges ? graph.edges.length : 0;
  if (edges > MAX_EDGES) {
    return fail(`graph too large (${edges} edges from ${nodes} definitions): too many ambiguous call names`);
  }
  let text;
  try {
    text = JSON.stringify(graph);
  } catch (err) {
    return fail(`graph too large to serialise (${edges} edges, ${nodes} definitions): ${err && err.message}`);
  }
  stream.write(text + '\n');
  return true;
}
