...
function tickFileCircles(vis, onlyFiles) {
  ...
  const visibleIds = vis || getVisibleNodeIds();   // global from main.js — safe at call time

  state.svgFileCircles.each(function(d) {
    // Frames engine passes the touched frames' files so a scheduler step only
    // refreshes the circles that can have moved.
    if (onlyFiles && !onlyFiles.has(d.filePath)) { return; }
